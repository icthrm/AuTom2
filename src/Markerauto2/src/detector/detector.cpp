...
//     std::cout<<"ok"<<std::endl;
    ...
    // Each rank processes its subset of slices
    std::vector<int> local_indices;
    for (int i = rank; i < mrcr.Size(); i += num_procs) {
        local_indices.push_back(i);
    }

    std::vector<std::vector<util::point2d> > local_fids(local_indices.size());

    for (size_t li = 0; li < local_indices.size(); ++li) {
        int i = local_indices[li];
        EX_TIME_BEGIN("%sProcessing MRC[%d] (rank %d)", _DASH, i, rank)
        ...
        detector.Process(slice, local_fids[li], true);

        EX_TIME_END("Processing MRC[%d] (rank %d)", i, rank)
    }

#ifdef HAVE_MPI
    // ---- Gather detected points to rank 0 ----

    // 1. Gather counts per slice
    std::vector<int> local_counts(mrcr.Size(), 0);
    for (size_t li = 0; li < local_indices.size(); ++li) {
        local_counts[local_indices[li]] = static_cast<int>(local_fids[li].size());
    }

    std::vector<int> global_counts(mrcr.Size(), 0);
    MPI_Reduce(local_counts.empty() ? NULL : &local_counts[0],
               global_counts.empty() ? NULL : &global_counts[0],
               mrcr.Size(), MPI_INT, MPI_SUM, 0, MPI_COMM_WORLD);

    // 2. Serialize local results
    int local_npoints = 0;
    for (size_t li = 0; li < local_fids.size(); ++li) {
        local_npoints += static_cast<int>(local_fids[li].size());
    }

    std::vector<util::point2d> local_points;
    local_points.reserve(local_npoints);
    std::vector<int> local_slices;
    local_slices.reserve(local_npoints);

    for (size_t li = 0; li < local_indices.size(); ++li) {
        int slice_idx = local_indices[li];
        for (size_t pi = 0; pi < local_fids[li].size(); ++pi) {
            local_points.push_back(local_fids[li][pi]);
            local_slices.push_back(slice_idx);
        }
    }

    // 3. Gatherv for points and slice indices
    std::vector<int> all_npoints(num_procs);
    MPI_Gather(&local_npoints, 1, MPI_INT,
               all_npoints.empty() ? NULL : &all_npoints[0], 1, MPI_INT, 0, MPI_COMM_WORLD);

    std::vector<int> displs(num_procs, 0);
    std::vector<util::point2d> all_points;
    std::vector<int> all_slices;
    int total_points = 0;

    if (rank == 0) {
        displs[0] = 0;
        total_points = all_npoints[0];
        for (int r = 1; r < num_procs; ++r) {
            displs[r] = displs[r - 1] + all_npoints[r - 1];
            total_points += all_npoints[r];
        }
        all_points.resize(total_points);
        all_slices.resize(total_points);
    }

    MPI_Datatype MPI_POINT2D;
    MPI_Type_contiguous(2, MPI_FLOAT, &MPI_POINT2D);
    MPI_Type_commit(&MPI_POINT2D);

    MPI_Gatherv(local_points.empty() ? NULL : &local_points[0], local_npoints, MPI_POINT2D,
                all_points.empty() ? NULL : &all_points[0],
                all_npoints.empty() ? NULL : &all_npoints[0],
                displs.empty() ? NULL : &displs[0], MPI_POINT2D,
                0, MPI_COMM_WORLD);
    MPI_Gatherv(local_slices.empty() ? NULL : &local_slices[0], local_npoints, MPI_INT,
                all_slices.empty() ? NULL : &all_slices[0],
                all_npoints.empty() ? NULL : &all_npoints[0],
                displs.empty() ? NULL : &displs[0], MPI_INT,
                0, MPI_COMM_WORLD);

    // 4. Assemble on rank 0
    if (rank == 0) {
        for (int k = 0; k < total_points; ++k) {
            fidsk->V(all_slices[k]).push_back(all_points[k]);
        }
    }

    // ---- Broadcast assembled fidstack back to all ranks ----
    std::vector<int> slice_displs(mrcr.Size() + 1, 0);
    std::vector<util::point2d> bcast_points;

    if (rank == 0) {
        for (int i = 0; i < mrcr.Size(); ++i) {
            slice_displs[i + 1] = slice_displs[i] + static_cast<int>(fidsk->V(i).size());
        }
        bcast_points.resize(slice_displs[mrcr.Size()]);
        for (int i = 0; i < mrcr.Size(); ++i) {
            int count = slice_displs[i + 1] - slice_displs[i];
            if (count > 0) {
                std::copy(fidsk->V(i).begin(), fidsk->V(i).end(), bcast_points.begin() + slice_displs[i]);
            }
        }
    }

    MPI_Bcast(slice_displs.empty() ? NULL : &slice_displs[0], mrcr.Size() + 1, MPI_INT, 0, MPI_COMM_WORLD);

    int bcast_total = slice_displs[mrcr.Size()];
    if (rank != 0) bcast_points.resize(bcast_total);
    MPI_Bcast(bcast_points.empty() ? NULL : &bcast_points[0], bcast_total, MPI_POINT2D, 0, MPI_COMM_WORLD);

    if (rank != 0) {
        for (int i = 0; i < mrcr.Size(); ++i) {
            int count = slice_displs[i + 1] - slice_displs[i];
            fidsk->V(i).resize(count);
            if (count > 0) {
                std::copy(bcast_points.begin() + slice_displs[i], bcast_points.begin() + slice_displs[i] + count, fidsk->V(i).begin());
            }
        }
    }

    MPI_Type_free(&MPI_POINT2D);
