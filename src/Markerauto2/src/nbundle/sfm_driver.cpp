...
//         Run(motstruct, projections, vmask, num_3d_pts, ncams, nconcam, fixed_pt_num);
        ...
        EX_TRACE("# SFM using %d 3D pts(%d fixed), %d frames(%d fixed) and %d image projections(%g p/p), (elapse: %ld)\n",
                 num_3d_pts, ncon3Dpts, ncams, nconcam, arr_idx, ((double)arr_idx) / num_3d_pts, EX_ELAPSE());
...
//     std::cout<<"ResErr:"<<residual/(arr_num)<<std::endl;
    std::cout << "residual:" << residual_tmp / (arr_num) << "   std:" << std << std::endl;
