...
inline void SeriesReadFromFile(cv::Mat &img, const char *filename)
{
    ...
    int tmp[4];                                               // width, height, type
    ifs.read(reinterpret_cast<char *>(tmp), sizeof(int) * 3); // 只需要读取前3个值

    // 创建矩阵
    img.create(tmp[1], tmp[0], tmp[2]); // height, width, type

    // 读取矩阵数据
    ifs.read(reinterpret_cast<char *>(img.data), img.elemSize() * img.total());
...
//     return result;
// }
inline cv::Mat GetStackImage(cv::Mat &image, cv::Rect roi)
{
    // 检查 ROI 是否在图像范围内
    if (roi.x < 0 || roi.x + roi.width > image.cols ||
        roi.y < 0 || roi.y + roi.height > image.rows)
    {
        return cv::Mat(); // 返回空矩阵而不是 NULL
    }
    ...
    // 使用 ROI 创建子图像
    cv::Mat result = image(roi).clone(); // 使用 clone() 创建深拷贝
